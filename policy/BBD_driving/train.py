...
tf.app.flags.DEFINE_string('train_dir', '/tmp/imagenet_train',
                           """Directory where to write event logs """#这里写事件logo 和训练检查点
                           """and checkpoint.""")
tf.app.flags.DEFINE_integer('max_steps', 10000000,# 几个批次
                            """Number of batches to run.""")
tf.app.flags.DEFINE_string('subset', 'train',#训练和验证模式
                           """Either 'train' or 'validation'.""")

# Flags governing the hardware employed for running TensorFlow.
tf.app.flags.DEFINE_integer('num_gpus', 1,#使用多少GPU训练
                           """How many gpus to use on the system"""
                           """Should be used with CUDA_VISIBLE_DEVICES""")

tf.app.flags.DEFINE_boolean('background_class', True,#图像分割背景类是否训练
...
def _tower_loss(inputs, outputs, num_classes, scope):
  ...
  multiple GPU's. For instance, if the batch size = 32 and num_gpus = 2,#一个批次有32张图片，两个gpu 分别处理16张图片
